...
describe('the message version gate', () => {
  it('accepts the version the types were built from', () => {
    expect(guard().checkMessage('status', '1.0')).toEqual({ ok: true, checked: true });
    expect(guard().checkMessage('error', '1.1')).toEqual({ ok: true, checked: true });
  });

  it('accepts a higher minor, which is an additive change', () => {
    expect(guard().checkMessage('status', '1.7').ok).toBe(true);
  });

  it('blocks a lower minor, where a field the types promise may be missing', () => {
    expect(guard().checkMessage('error', '1.0')).toMatchObject({
      ok: false,
  ...
  it('blocks a higher major and points at the extension', () => {
    expect(guard().checkMessage('status', '2.0')).toMatchObject({
      ok: false,
  ...
  it('checks a progress message against its final kind', () => {
    expect(guard().checkMessage('sync-progress', '1.0')).toEqual({ ok: true, checked: true });
  });

  it('passes a kind it has no contract for, and says it did not check', () => {
    expect(guard().checkMessage('user-add', '3.0')).toEqual({ ok: true, checked: false });
  });

  it('blocks a version string that is not major.minor', () => {
    expect(guard().checkMessage('status', '1.0.0')).toMatchObject({
      ok: false,
      problem: 'unparsable',
    });
  });
